...
- **CI/CD (GitHub Actions):** on PR/push — typecheck, lint, unit tests, and **golden tests using the synthetic adapter only** (hermetic: no model download, no network, no Kokoro). The Kokoro path is exercised in a separate, optional, manually-triggered job (or on tags) where the model download is acceptable. Release is a tag → `npm publish` (provenance enabled).
...
└── .github/workflows/    # golden.yml (hermetic, synthetic-only) + release
```
